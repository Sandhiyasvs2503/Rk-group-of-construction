      ...
      srvGrid.innerHTML = cms.services.map(s => `
        <div class="service-card">
          <div class="service-img-wrap">
            <img src="${escapeHTML(s.img || './assets/images/commercial_tower.jpg')}" alt="${escapeHTML(s.title)}" class="service-card-img">
            <div class="service-icon-badge"><i class="fa-solid ${escapeHTML(s.icon || 'fa-building')}"></i></div>
          </div>
...
function initModals() {
  ...
  // Universal Event Delegation for Call, Location, Quote Modals and Close Handlers
  document.addEventListener('click', (e) => {
    // 1. Quote Modal Trigger
    const quoteBtn = e.target.closest('.open-quote-modal');
    if (quoteBtn) {
      e.preventDefault();
    ...
    // 2. Call Modal Trigger
    const callBtn = e.target.closest('#open-call-modal, .open-call-modal, .float-phone');
    if (callBtn) {
      e.preventDefault();
      document.querySelectorAll('.modal-backdrop.show').forEach(m => m.classList.remove('show'));
    ...
    // 3. Location Modal Trigger
    const locBtn = e.target.closest('#open-location-modal, .open-location-modal, .float-location');
    if (locBtn) {
      e.preventDefault();
      document.querySelectorAll('.modal-backdrop.show').forEach(m => m.classList.remove('show'));
    ...
    // 4. Universal Modal Close Handler (X buttons, Cancel buttons, and Backdrop clicks)
    const closeBtn = e.target.closest('.modal-close, #cancel-quote-modal, #close-proj-modal, #close-quote-modal, #close-call-modal, #close-location-modal, #close-admin-modal');
    if (closeBtn) {
      e.preventDefault();
      e.stopPropagation();
      document.querySelectorAll('.modal-backdrop.show').forEach(m => m.classList.remove('show'));

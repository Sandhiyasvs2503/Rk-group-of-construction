...
async function fetchSupabaseData() {
  ...
  try {
    ...
    // Format projects with gallery arrays
    ...
    return {
      hero: {
        tag: settings?.hero_tag,
        title: settings?.hero_title,
        sub: settings?.hero_subtitle,
        heroImg: settings?.hero_image_url,
        liveSafety: settings?.live_safety,
        liveBca: settings?.live_bca,
        liveOntime: settings?.live_ontime,
        phone: settings?.phone,
        email: settings?.email
      },
      categories: (categories || []).map(c => ({ id: c.id, key: c.key, label: c.label })),
      projects: projectsWithGallery,
      groupEntities: (groupEntities || []).map(g => ({ id: g.id, tag: g.tag, title: g.title, icon: g.icon, desc: g.description, f1: g.f1, f2: g.f2, f3: g.f3 })),
      services: (services || []).map(s => ({ id: s.id, title: s.title, desc: s.description, icon: s.icon, img: s.image_url })),
      testimonials: (testimonials || []).map(t => ({ id: t.id, name: t.name, role: t.role, stars: t.stars, quote: t.quote }))
    };
  } catch (err) {
...
async function renderCMSContent() {
  const cms = await fetchSupabaseData();
  if (!cms) return;
...
function initProjectLightbox() {
  ...
  document.querySelectorAll('.view-proj-btn').forEach(btn => {
    btn.onclick = (e) => {
      ...
      const rawGal = btn.getAttribute('data-gallery');

...
function initModals() {
  ...
  // Universal Event Delegation for Quote Modal Triggers (including dynamic buttons inside Lightbox)
  document.addEventListener('click', (e) => {
    const trigger = e.target.closest('.open-quote-modal');
    if (trigger) {
      e.preventDefault();
      const projModal = document.getElementById('project-lightbox-modal');
      if (projModal) projModal.classList.remove('show');
      const quoteModal = document.getElementById('quote-modal');
      if (quoteModal) quoteModal.classList.add('show');
      return;
    }

    // Universal Modal Close Handler (X buttons, Cancel buttons, and Backdrop clicks)
